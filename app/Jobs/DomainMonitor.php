...
class DomainMonitor implements ShouldQueue
{
    ...
    public function handle()
    {
        //
        try {
            $ping = Http::head('http://' . $this->domain->name);
            $crawlerError = false;
        } catch (Exception $e) {
            $crawlerError = true;
        }
        ...
        if($crawlerError == false) {
            if($ping->successful()) {
                // dispatch domain monitor
                $this->doDomainMonitorTasks($this->domain, self::TYPE_UP);
            } else {
                $this->doDomainMonitorTasks($this->domain, self::TYPE_DOWN);
            }
        } else {
            $this->doDomainMonitorTasks($this->domain, self::TYPE_DOWN);
        }
    ...
    private function sendTwiloNotification($domain, $type, $method) {
        ...
        if($method == self::TYPE_DOWN) {
            // Actually wait more 5 minutes if still down.
